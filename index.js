...
const getSystemInstructions = (style, tone) => {
  const styleText = style ? `Стиль письма: ${style}` : '';
  const toneText = tone ? `Тон письма: ${tone}` : '';
...
app.post('/getContent', async (req, res) => {
  try {
    const { topic, description, style, tone } = req.body;
    ...
    const result = await gemini.generateContentStream({
      ...
      generationConfig,
      safetySettings,
      systemInstruction: getSystemInstructions(style, tone),
    });
